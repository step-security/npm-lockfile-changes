...
const run = async () => {
  try {
    ...
    const lockFileName = path.basename(inputPath)
    const baseLockFile = baseTree.data.tree.find((file) => file.path === lockFileName)

    if (!baseLockFile) {
      warning('💡 No lock file found on base branch — skipping diff.')
      return
    }

    const baseLockSHA = baseLockFile.sha
